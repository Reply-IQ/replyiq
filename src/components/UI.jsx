...
// ── INPUT ─────────────────────────────────────────────────────────────────────
export function Input({ label, value, onChange, type='text', placeholder, prefix, suffix, style, step, min, max, required, hint }) {
  return (
    <div style={style}>
      ...
      {hint && <div style={{ fontSize:'11px', color:'var(--text3)', marginTop:4, lineHeight:1.4 }}>{hint}</div>}
    </div>
  )
}
...
export function Alert({ type='warning', title, children, style }) {
  const s = AS[type] || AS.warning
  return (
    <div style={{ display:'flex', gap:12, padding:'13px 16px', borderRadius:'var(--r-md)', background:s.bg, border:`1px solid ${s.border}`, marginBottom:10, ...style }}>
...
export function TrialBanner({ daysLeft, aiUsed, aiLimit, aiRemaining, onUpgrade }) {
  ...
  return (
    <div style={{ background:urgent?'rgba(184,92,56,0.08)':'rgba(201,169,110,0.06)', border:`1px solid ${urgent?'rgba(184,92,56,0.25)':'rgba(201,169,110,0.18)'}`, borderRadius:'var(--r-md)', padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
      <div style={{ display:'flex', alignItems:'center', gap:14, flex:1, minWidth:0 }}>
        <div style={{ flexShrink:0 }}>
          ...
          <div style={{ fontSize:'12px', color:'var(--text2)' }}>{aiRemaining} of {aiLimit} AI generations remaining</div>
        </div>
      ...
      <button onClick={onUpgrade} style={{ background:'linear-gradient(135deg,#C9A96E,#b8945a)', border:'none', borderRadius:8, padding:'8px 18px', color:'#141920', fontSize:'12px', fontWeight:700, cursor:'pointer', fontFamily:'var(--font-sans)', flexShrink:0, boxShadow:'0 2px 12px rgba(201,169,110,0.2)' }}>
        Upgrade Now →
      </button>
    </div>
  )
...
export function UpgradeModal({ onClose, onCheckout, reason }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(20,25,32,0.85)', backdropFilter:'blur(8px)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'#1C2430', border:'1px solid #243044', borderRadius:'var(--r-xl)', padding:'40px 36px', maxWidth:500, width:'100%', position:'relative' }}>
        ...
        <div style={{ display:'flex', gap:12, marginBottom:24 }}>
          {[
            ...
            <button key={p.plan} onClick={() => onCheckout(p.plan)}
              ...
              onMouseLeave={e => e.currentTarget.style.borderColor=p.highlight?'rgba(201,169,110,0.35)':'#243044'}>
              <div style={{ fontSize:'10px', color:'#C9A96E', fontWeight:700, textTransform:'uppercase', letterSpacing:'1px', marginBottom:10 }}>{p.badge}</div>
              <div style={{ fontFamily:'var(--font-serif)', fontSize:'1.6rem', color:'#C9A96E', marginBottom:4 }}>{p.price}</div>
              <div style={{ fontSize:'12px', color:'var(--text3)', marginBottom:8 }}>{p.period}</div>
              <div style={{ fontSize:'13px', fontWeight:700, color:'var(--text1)' }}>{p.label}</div>
            </button>
          ))}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:16 }}>
          {[
            ...
            'All platforms: Google, TripAdvisor, Booking, Instagram, Facebook',
